...
在适用法律允许的最大范围内，JustRun 按“现状”和“可用”提供，不提供任何明示或默示保证。使用者应自行承担使用本项目的全部风险、后果和法律责任；因未经授权的使用、绕过第三方安全控制、数据损失、服务中断、兼容性问题或其他原因产生的损失，与作者和维护者无关。高价值逻辑仍应服务端化，不能把本项目理解为绝对反逆向或绝对安全方案。
